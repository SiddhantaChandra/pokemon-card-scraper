...
func (bs *BadgerStorage) ClearAllData() error {
	log.Println("Starting database reset - clearing card data only (preserving trackers)...")
	...
	return bs.db.Update(func(txn *badger.Txn) error {
		...
		// Collect only card-related keys (preserve tracker data)
		for iterator.Rewind(); iterator.Valid(); iterator.Next() {
			item := iterator.Item()
			key := item.Key()
			keyStr := string(key)

			// Only delete card-related keys and indexes, preserve tracker data
			if strings.HasPrefix(keyStr, "card:") || strings.HasPrefix(keyStr, "idx:") {
		...
		log.Printf("Successfully deleted %d card records from database (preserved tracker data)", len(keysToDelete))
		return nil
	})
}
